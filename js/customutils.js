const NaoKuo = {
    ...
    try {
      if (!ipLoacation) {
        return new Promise((resolve, reject) => {
          ...
          var url = `https://apis.map.qq.com/ws/location/v1/ip?key=W23BZ-3LICT-LDWXD-LJRGP-F72KJ-JVFLW&output=jsonp`;
          script.src = url;
  ...
  showWelcome: (ipStore) => {
    ...
    let dist = NaoKuo.getDistance(31.473331,104.756915, ipStore.result.location.lng, ipStore.result.location.lat),
      address,
      welcome_info;
    //根据国家、省份、城市信息自定义欢迎语
    //海外地区不支持省份及城市信息
    ...
    🗺️您距离 <strong>槿鸢</strong> 约有 <strong>${dist}</strong> 公里！<br>
    当前IP地址为：<br>
